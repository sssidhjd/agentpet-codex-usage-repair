...
// Codex v2 sends cumulative snapshots, not fire-and-forget token deltas. The
// receipt and pet XP commit together; repeats/restarts do not re-feed history.
listen<care.CodexUsageSnapshot & { session: string; project: string }>("codex-usage-snapshot", (e) => {
  const p = e.payload;
  if (!p || !ownsProject(p.project)) return;
  const slug = myPetSlug();
  if (!slug) return;
  try {
    const tokens = care.applyCodexSnapshot(slug, p);
    if (tokens <= 0) return;
    emit("care-updated");
    sync.schedulePush();
    evaluateCareMetrics();
    if (p.project) usage.recordTokens(p.project, "codex", tokens, 0);
  } catch (error) {
    console.error("[codex-usage] Could not persist receipt; will retry", error);
  }
});
