...
fn handle_event(app: &AppHandle, body: &str) {
    ...
    // Respect the hook's transcript path. A resumed thread may have several
    // rollout files with the same session id; the legacy permanent first-match
    // cache kept reading an old one. The v2 watcher retries cumulative snapshots.
    if agent_kind == "codex" {
        let sess = tok_session.clone();
        let proj = tok_project.clone();
        std::thread::spawn(move || {
            crate::codex_usage::track(&sess, &proj, &transcript);
        });
    }
